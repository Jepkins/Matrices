...
static const int ELEMENTS_LENGTH = 12;
...
void mat_print (const matrix* a)
{
    for (size_t i = 0; i < a->size_x; i++)
    {
        ...
        for (size_t j = 0; j < a->size_y; j++)
            printf("%*lg ", ELEMENTS_LENGTH, *(a->arr + i*a->size_y + j));
...
int mat_submat (matrix* dst, const matrix* a, const size_t i, const size_t j)
{
...
int mat_adjugate_mat (matrix* a)
{
    ...
    for (size_t i = 0; i < a->size_x; i++)
        for (size_t j = 0; j < a->size_y; j++)
            adj.arr[i*adj.size_y + j] = mat_adjugate(a, i, j);
...
int mat_invert (matrix* a)
{
    ...
    if (isnan(det) || is_zero(det) || mat_adjugate_mat(a) == 0)
        return 0;
...
double mat_minor(const matrix* a, const size_t i, const size_t j)
{
    ...
    matrix com = {};
    mat_ctor(&com, a->size_x - 1, a->size_y - 1);

    mat_submat(&com, a, i, j);
...
{
    return (((i+j) % 2) ? -1 : 1) * mat_minor(a, i, j);
}
...
int mat_mult(const matrix* a, const matrix* b, matrix* c)
{
    ...
    size_t size_b = b->size_y;
    size_t size_n = a->size_y;
